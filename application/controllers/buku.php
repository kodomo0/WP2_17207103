...
class Buku extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('judul_buku', 'JudulBuku', 'required|min_length[3]', ['required' => 'Judul Buku harus diisi','min_length' => 'Judul buku terlalu pendek']);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = ['judul_buku' => $this->input->post('judul_buku',true),'id_kategori' => 
...
public function ubahBuku()
{
    ...
    $this->form_validation->set_rules('judul_buku', 'JudulBuku', 'required|min_length[3]', ['required' => 'Judul Buku harus diisi','min_length' => 'Judul buku terlalu pendek']);
    $this->form_validation->set_rules('id_kategori', 'Kategori','required', ['required' => 'Nama pengarang harus diisi',]);
    $this->form_validation->set_rules('pengarang', 'NamaPengarang', 'required|min_length[3]', ['required' => 'Nama pengarang harus diisi','min_length' => 'Nama pengarang terlalu pendek']);\
    ...
    if ($this->form_validation->run() == false) {
        ...
        if ($this->upload->do_upload('image')) {
            $image = $this->upload->data();unlink('assets/img/upload/' . $this->input->post('old_pict', TRUE));
            $gambar = $image['file_name'];
        } else {
    ...
    $this->input->post('id')]);redirect('buku');
}
}
