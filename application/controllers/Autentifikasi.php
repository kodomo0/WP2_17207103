...
class Autentifikasi extends CI_Controller
{
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    if ($user['role_id'] == 1) {redirect('index.php/admin');
                    } else {
 ...
 {
     if ($this->session->userdata('email')) {redirect('index.php/user');
     }
     ...
     if ($this->form_validation->run() == false) {
         ...
         $data = ['nama' => htmlspecialchars($this->input->post('nama', true)),'email' => htmlspecialchars($email),'image' => 'default.jpg','password' => password_hash($this->input->post('password1'), PASSWORD_DEFAULT),'role_id' => 1,'is_active' => 1,'tanggal_input' => time()];
         $this->modeluser->simpanData($data); 
         //menggunakan model
         $this->session->set_flashdata('pesan', '<div class="alert alert-success alert-message" role="alert">Selamat!! akun member anda sudah dibuat. Silahkan Aktivasi Akun anda</div>'); redirect('index.php/autentifikasi'); 
     }
 }
}
